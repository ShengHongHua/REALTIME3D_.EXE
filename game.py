...
class RayCastingGame:
    """
    2.5次元环境 - 光线投射/末日风格。
       该游戏根据矩形块的地图计算并绘制光线投射环境。
    每个墙壁、地板和天花板像素都使用 pygame 表面像素阵列使用 NumPy 进行映射和绘制。
    硬币和灯泡图像被添加到顶部。绘制地图以帮助导航。   随着时间的流逝，你的灯会变暗——除非你收集灯泡。
    当能见度几乎为零时，游戏结束。
    键：光标键用于转向，A 和 Z 键用于调整速度。
    ...
    def __init__(self, screen, target_fps):
        ...
        self.moving_time = 0
        self.anim_time = 0
        self.angle_speed = 18  # 定义转弯速度 defines turning speed
        self.channel = 1
        self.nr_channels = 8
        self.running = True
        self.paused = False
        ...
        self.pic_theend = pygame.image.load('data\image\TheEnd.jpg').convert()
        self.pic_guru2 = pygame.image.load('data\image\Guru2.jfif').convert()
        self.pic_ball2 = pygame.image.load('data\image\Ball2.jpg').convert()
        self.pic_alphaks2 = pygame.image.load('data\image\Alphaks2.jfif').convert()
        self.pic_vector3d = pygame.image.load('data\image\Vector3D.jfif').convert()
        self.pic_shadowbobs = pygame.image.load('data\image\ShadowBobs.jpg').convert()
        self.pic_milkyway2 = pygame.image.load('data\image\Milky_Way2.jpg').convert()
        self.pic_ray01 = pygame.image.load('data\image\Raytracing_01.jpg').convert()
        self.pic_ray02 = pygame.image.load('data\image\Raytracing_02.jpg').convert()
        self.pic_ray03 = pygame.image.load('data\image\Raytracing_03.jpg').convert()
        self.pic_ray04 = pygame.image.load('data\image\Raytracing_04.jpg').convert()
        self.pic_ray05 = pygame.image.load('data\image\Raytracing_05.jpg').convert()
        self.pic_ray06 = pygame.image.load('data\image\Raytracing_06.jpg').convert()
        self.pic_ray07 = pygame.image.load('data\image\Raytracing_07.jpg').convert()
        self.pic_ray08 = pygame.image.load('data\image\Raytracing_08.jpg').convert()
        self.pic_ray09 = pygame.image.load('data\image\Raytracing_09.jpg').convert()
        self.pic_ray10 = pygame.image.load('data\image\Raytracing_10.jpg').convert()
        self.pic_coin = pygame.image.load('data\image\goldcoin2.png').convert()
        self.pic_coin.set_colorkey((0, 0, 0))
        self.pic_lightbulb = pygame.image.load('data\image\lightbulb3.png').convert()
        self.pic_lightbulb.set_colorkey((0, 0, 0))

        self.sound_collide = pygame.mixer.Sound('data\music\RayCasting_Game_Ray_cast_collide.wav')
        self.sound_bounce = pygame.mixer.Sound('data\music\RayCasting_Game_Ray_cast_bounce.wav')
        self.sound_start = pygame.mixer.Sound('data\music\RayCasting_Game_Ray_cast_start.wav')
        self.sound_pling = pygame.mixer.Sound('data\music\RayCasting_Game_Ray_cast_pling.wav')
        self.sound_lose = pygame.mixer.Sound('data\music\RayCasting_Game_Ray_cast_lose.wav')

        self.wall_blocks = []   # 可用块列表（类块） list of available blocks (class Block)
        self.map_blocks = {}    # 将地图块（self.map 中的字母数字字符）连接到列表索引wall_blocks字典 dictionary connecting map block (alphanumeric character in self.map) to wall_blocks list index
        self.items = []         # 项目列表（类项目） list of items (class Item)
        self.setup_blocks()
        self.coin_blocks = 'cdefg'
        self.light_block = 'L'
        self.map = ['11111111111111111111111111111111111111111111111',
                    '1L  f                           2222         L1',
                    '1   f   1f     L      efgfedcde  22L   ceg L  1',
                    '1   e  13f11     22              66    gec    1',
                    '1   e  1ee1      22      55                   3',
                    '1   d   d41           L      efefef    131    3',
                    '1  6d        L1                        131    3',
                    '1     ccccddee      11       2111111          3',
                    '1                            2                3',
                    '1   f         g        2  c   2    4444    f  1',
        ...
        self.map_minimum_size = np.array([15, 15])  # 地图不会减小到比这更小的尺寸map will not decrease to smaller size than this
        self.position = self.map_size / 2  # 初始位置居中initial position at center
        self.pos_angle = 0.0
        self.view_angle = 0.0
        self.view_height = 0.5  #眼睛高度占视野的百分比 "eye height" as percentage of view
        self.view_width = 60    #以度为单位查看宽度角度（水平方向） view width angle in degrees (horizontally)
        self.wall_height = self.width  # 这将调整墙壁高度。this adjusts wall height.
        self.stuck = True

        self.view_blocks = 5.5   # how many blocks away can see
        self.view_blocks_target = self.view_blocks
        self.view_lightbulb_add = 6.5  # 通过获得灯泡添加多少个视图块 how many blocks of view are added by getting a light bulb
        self.view_blocks_decrease = 20  # 一个区块减少多少秒view_blocks how many seconds for a one block decrease in view_blocks
        self.view_blocks_time = 0
        self.view_blocks_target_time = 0
        self.view_shade = 0.5   # 视图的分数，之后开始着色 fraction of view after which shading starts
        self.map_show_size = np.minimum(self.map_size, self.view_blocks * 2).astype(np.int16)  # 显示的地图区域大小 - 限制为 2 x view_blocks the size of map area shown - limit to 2 x view_blocks

        self.proj_dist = np.tan(self.view_width * np.pi / 180) * self.width
        self.angle_add = self.view_width / self.width * np.pi / 180

        # 从 Item 类复制项目数据以加快处理速度 item data copied from Item class to speed up handling
        ...
        self.setup_items()
        self.item_data = np.zeros((len(self.items), 6), dtype=np.float)  # 项目数据具有可见的项目编号、角度、距离、x_coord、y_floor y_ceilingitem data has visible item nr, angle, distance, x_coord, y_floor, y_ceiling
        ...
        # 定义天花板和地板图片 define ceiling and floor pics
        ...
        # ray_data 是网格项的数组，侧（0 = 向上，1 = 右，2 = 向下，3 = 左）映射 X，映射 Y， ray_data is an array of grid item, side (0 = up, 1 = right, 2 = down, 3 = left) map x, map y,
        #  与观察者的距离，鱼缸的距离校正，每条光线的y_top和y_bottom distance from viewer, distance corrected for fishbowl, y_top and y_bottom for each ray
        self.ray_data = np.zeros((self.width, 8), dtype=np.float)
        # 每个角度的距离校正乘数以抵消鱼缸效应 distance correction multiplier for each angle to counter the fishbowl effect
        self.distance_correction = np.cos(np.linspace(-self.view_width / 2, self.view_width / 2, self.width) / 180.0 * np.pi)
        # grid_blocks将保存每个射线块（用于单个处理运行）开始和结束位置的数据 grid_blocks will hold the data on where each block of rays (for single processing run) starts and ends
        ...
        # 设置计时器 set up timers
        self.timer_name = []
        # self.timer_names.append("clear") #清楚
        self.timer_names.append("calculate") #算
        self.timer_names.append("draw walls") #绘制墙壁
        self.timer_names.append("draw ceiling") #绘制天花板
        self.timer_names.append("draw floor") #拉丝地板
        self.timer_names.append("item handling") #物料搬运
        self.timer_names.append("draw map") #绘制地图
        self.timer_names.append("plot info") #剧情信息
        self.timer_names.append("display flip") #显示器翻转
        self.timer_names.append("wait") #等
        self.timers = np.zeros((len(self.timer_names), self.timer_avg_frames), dtype=int)

        # 初始化计时器 initialize timers
    ...
    def run(self):
        ...
        # 以全屏模式启动 start in full screen mode
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_s:
                        # 保存屏幕，分辨率为一半的分辨率，使用类名作为文件名 save screen, at half the resolution, using class name as file name
            ...
            # 计算视图中的墙壁 calculate the walls in view
            self.raycast(self.screen, self.block_size)

            # 绘制操作，清除是绘图墙的一部分，因此不需要单独使用。地板和天花板两种选择：整体或可见 drawing operations; clear is part of drawing walls so not required separately. Two options for floor and ceiling: "whole" or "visible"
            ...
            # 释放屏幕上的任何锁定 release any locks on screen
            while self.screen.get_locked():
                self.screen.unlock()

            # 在当前显示和下一个屏幕之间切换（在缓冲区中准备） switch between currently showed and the next screen (prepared in "buffer")
            pygame.display.flip()
            self.measure_time("display flip")
            self.clock.tick(self.target_fps)  # 这使代码以最大target_fps运行 this keeps code running at max target_fps
            self.measure_time("wait")
            self.next_time_frame()

    def adjust_view_blocks(self, time):

        # 随着时间的流逝自动降低view_blocks automatically decrease view_blocks as time passes
        ...
        if self.view_blocks <= 1 and not self.game_over:
            # 使用view_blocks_target仅逐渐更改实际view_blocks visibility gone --> game over
            self.game_over = True
    ...
    def animate(self):
        ...
        # 为墙面图像制作动画;保持速度恒定，帧速率不同。animate wall images; keep speed constant with frame rate differing.
    ...
    def move(self, keys, time):
        ...
        # 调整移动时间以在帧速率不同时保持速度恒定。adjust move for time to keep speed constant with frame rate differing.
        # 如果游戏结束，请将速度调整为零。if game over, adjust speed to zero.
        ...
        self.move_speed = max(-2.0, min(10.0, self.move_speed + speed * move_time_adj))  # 将速度保持在此范围内 keep speed in this range
        turn_speed = max(self.move_speed, 3.0)  # 但是对于转弯和向上/向下移动，请使用最低 3.0 but for turning and moving up/down use minimum 3.0

        # 向上/向下移动 moving up/down
        ...
        bounce_angle = 30  # 如果碰撞角度小于此角度，则反弹 if collision angle is less than this, then bounce
        # 检查新位置是否不在墙内（非零块）或到墙的距离非常小（途中的拐角） check that new position is not within a wall (a non-zero block) OR that the distance to wall is very small ("corner" on the way)
        ...
        if self.map_array[int(new_pos[0]), int(new_pos[1])] != 0 or self.ray_data[self.width // 2, 4] < self.move_speed * move_time_adj:
            # 墙内的新位置。检查网格的角度是否最大bounce_angle度 new position inside a wall. Check if angle to grid is max bounce_angle degrees
            new_angle = -999.0
            if (self.pos_angle + bounce_angle) % 180 <= 2 * bounce_angle and self.ray_data[self.width // 2, 1] in (1, 3):
                # 向上或向下移动，墙壁是左侧或右侧墙壁 - 将反弹应用于 X 坐标 going up or down and the wall is left or right side wall - apply bounce to x coordinate
                new_pos[0] += 2 * (round(new_pos[0], 0) - new_pos[0])
                new_angle = self.pos_angle - 2 * ((self.pos_angle + bounce_angle) % 180 - bounce_angle)  # 弹跳角度 "bounce angle"
            elif (self.pos_angle + 90 + bounce_angle) % 180 <= 2 * bounce_angle and self.ray_data[self.width // 2, 1] in (0, 2):
                # 向左或向右移动，墙壁向上或向下侧壁 - 将反弹应用于 y 坐标 going left or right and the wall is up or down side wall - apply bounce to y coordinate
                new_pos[1] += 2 * (round(new_pos[1], 0) - new_pos[1])
                new_angle = self.pos_angle - 2 * ((self.pos_angle + 90 + bounce_angle) % 180 - bounce_angle)  # 弹跳角度 "bounce angle"
            # 检查墙壁的反弹位置，如果卡住则不要应用反弹 check bounced position for walls, do not apply bounce if stuck
            if not self.stuck and self.map_array[int(new_pos[0]), int(new_pos[1])] == 0 and new_angle > -999.0:
                # 反弹可能。降低速度并设置新角度 bounce possible. Decrease speed and set new angle
                self.move_speed *= 0.7
                self.pos_angle = new_angle
                self.play_sound(self.sound_bounce)
            else:
                # 没有弹跳，卡住。尝试反弹。 no bounce, get "stuck". Try to "bounce back".
                new_pos = self.position - np.array([sp, cp]) * self.move_speed * 0.025
                if self.map_array[int(new_pos[0]), int(new_pos[1])] != 0:
                    # 反弹不可能，只是卡住了... bounce back not possible, just stuck...
    ...
    def raycast(self, screen, block_size):
        ...
        # 水平浏览视图，即从位置和view_angle从左view_angle_witdh / 2到右view_angle_witdh / 2.
        # 在水平和垂直下方是指屏幕上的地图，而不是视图本身。
        # 角度 0 度 = 向上，顺时针旋转
        # position rounded to grid and the decimals (ie. position within the grid it is in)
        # 位置四舍五入到网格和小数（即它所在的网格内的位置）
        pos = np.floor(self.position)
        pos_dec = self.position - pos

        width = self.width  # 这是投射的光线数 - 理想情况下是程序窗口的宽度（以像素为单位），因此每垂直像素线一条光线。 this is the number of rays cast - ideally the width of the program window in pixels, so one ray per vertical line of pixels.
        view = int(self.view_blocks)  # 块的nr：可见的距离（因此，必须计算多远） nr of blocks: how far is visible (and, hence, how far must be calculated)
        rng_width = np.arange(width)
        # rng_view从零到最大视图长度（以块为单位）。 rng_view goes from zero to maximum view length in blocks.
        rng_view = np.arange(view + 1)  # 添加 1 以确保在块向下舍入时覆盖可视距离 add 1 to make sure covers the viewable distance as blocks are "rounded down"
        # 计算所有角度等。一气呵成 - 比循环更快 calculate all angles etc. in one go - faster than looping
        ...
        # rads 是以弧度为单位的所有视角的数组（大小：宽度）。
        # 添加 0.0001 以避免精确的 0/90/180/270 度（因此 0 sin/cos - div 乘以零）rad 的间距不均匀，因为它们在一条线上，而不是圆半径上。
        # 它们的切线均匀分布（当尚未应用view_angle时），
        rad_tan = np.tan(self.view_width * np.pi / 360)  # rad_tan是一半视角的切线 rad_tan is the tangent of half the viewing angle
        rads = np.arctan((rng_width / (width - 1) - 0.5) * rad_tan * 2) + self.view_angle * np.pi / 180 + 0.0001
        sin_a = np.sin(rads)
        cos_a = np.cos(rads)
        tan_a = sin_a / cos_a  # np.tan(rads)
        sign_x = np.sign(sin_a)   # 角度 0 和 180 sign_x = +1，角度 180 为 -1 sign_x = +1 for angles > 0 and < 180, -1 for angles > 180
        sign_y = np.sign(cos_a)   # 角度 90 或 270 sign_y = +1，角度 90 和 270 为 -1 sign_y = +1 for angles < 90 or > 270, -1 for angles > 90 and < 270
        ...
        # 获取沿射线线移动到下一个网格时遇到的第一个坐标如果sign_y为负，则向下投射光线，y 距离为 pos_dec1 - 1。同样的x...
        ...
        # 射线和网格的垂直交点，y 坐标增加/减少 1，计算各自的 x coordinatex_coord_y 是网格点（没有网格哪一侧的信息），而 x_coord_y_adj 是针对相对于它的观察者位置调整的 y 坐标（包括侧）.
        # 即如果网格点高于查看器 （sign_y = -1）， 添加 1 以将x_coord_y_adj带到该网格点的底部。
        ...
        # 从地图中获取相应的格网像元数据。确保不要超过地图边界。只选择块的视图r，en添加一个停止块，因为最后一个块顶部块是-1，因为它不是真正的块。
        vert_grid = np.hstack((
            ...
            -np.ones((width, 1), dtype=np.int16)  # 这是最远的块，始终标记为 1 以确保 Argmax 找到某些东西。this is the farthest block and always marked as "1" to make sure argmax finds something.
            ))
        ...
        # 查找每个角度的第一个非零项索引。由于距离随着每个项目而增加，因此这是每条光线遇到的最近墙。
        ...
        # 构造网格项、边（0 = 向上、1 = 右、2 = 向下、3 = 左）映射 X、地图 Y 和与查看器的距离的数组
        vert_data = (np.vstack((
        ...
        for row in range(self.map_size[1]):
            ...
            for col in range(self.map_size[0]):
                if self.map_array[col, row] > 0:
                    pos = np.array([col * block_size, row * block_size], dtype=int)
...
if __name__ == '__main__':
    ...
    pygame.font.init()
    pygame.mixer.init()
    music_file = "data\music\\alacrity.mod"  # this mod by Jellybean is available at e.g. http://janeway.exotica.org.uk/
    jls_extract_var = music_file
    pygame.mixer.music.load(jls_extract_var)
